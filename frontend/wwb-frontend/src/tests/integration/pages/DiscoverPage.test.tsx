...
describe('DiscoverPage', () => {
  ...
  it('loads indicators and allows search by name', async () => {
    ...
    const searchInput = screen.getByPlaceholderText(/search indicators by name or description/i)

    fireEvent.change(searchInput, { target: { value: 'Femicide' } })

    await waitFor(() => {
      expect(screen.getByText(/femicide rate/i)).toBeInTheDocument()
      expect(screen.queryByText(/women, business and the law index/i)).not.toBeInTheDocument()
    })
  })

  it('shows error card if loading fails', async () => {
    vi.mocked(indicatorsApi.getIndicators).mockRejectedValueOnce(new Error('Backend is down'))
