...
describe('ReferencesPage', () => {
  it('renders data sources list and filters by search', () => {
    ...
    expect(screen.getByText(/Data Sources & Licenses/i)).toBeInTheDocument()

    expect(screen.getByText(DATA_SOURCES[0].name)).toBeInTheDocument()

    const searchInput = screen.getByPlaceholderText(/Search data sources/i) as HTMLInputElement
    ...
    expect(screen.getByText(/No data sources found matching your search/i)).toBeInTheDocument()
