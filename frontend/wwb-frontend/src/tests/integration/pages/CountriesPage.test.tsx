...
describe('CountriesPage', () => {
  ...
  it('loads countries, featured indicators and renders country cards', async () => {
    ...
    expect(await screen.findByText(/Country Profiles/i)).toBeInTheDocument()
    ...
    await waitFor(() => {
      expect(screen.getByText(/View Women, Business and the Law index/i)).toBeInTheDocument()
    })
  })
  ...
  it('shows error card when countries request fails', async () => {
    ...
    mockGet.mockImplementation((url: string) => {
      if (url === '/locations') {
        throw new Error('Countries API is down')
      }
      if (url === '/indicators') {
        return Promise.resolve({
          data: { total: 0, items: [] },
