...
vi.mock('@/pages/Home/entities/home.constants', async () => {
    const actual = await vi.importActual<
        typeof import('@/pages/Home/entities/home.constants')
...
describe('HomePage', () => {
    it('renders hero title and CTA buttons', async () => {
        ...
        expect(
            screen.getByText(/women's well-being evaluation platform/i),
        ).toBeInTheDocument()

        expect(
            screen.getByRole('link', { name: /explore data/i }),
        ).toBeInTheDocument()

        expect(
            screen.getByRole('link', { name: /read methodology/i }),
        ).toBeInTheDocument()
    })
    ...
    it('uses default badge style when data source has unknown color', async () => {
        ...
        expect(
            screen.getByText(/Fallback badge/i),
        ).toBeInTheDocument()
    })
})
